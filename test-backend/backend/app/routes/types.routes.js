const { authJwt } = require("../middleware");
const { Op } = require("sequelize");
const { checkDuplicate } = require("../utils");

const db = require("../models");

module.exports = function (app) {
  app.use(function (req, res, next) {
    res.header(
      "Access-Control-Allow-Headers",
      "x-access-token, Origin, Content-Type, Accept"
    );
    next();
  });

  app.get("/api/vehicle-types", [authJwt.verifyToken], (req, res) => {
    let limit = parseInt(req.query.limit) || 2;
    let offset = parseInt(req.query.offset) || null;

    let name = req.query.name;
    let brand_id = parseInt(req.query.brand_id);

    const query = {};
    if (name) query.name = { [Op.like]: `%${name}%` };
    if (brand_id) query.brand_id = { [Op.eq]: brand_id };

    db.vehicle_type
      .findAndCountAll({
        where: query,
        limit: limit,
        offset: offset,
        order: [["id", "ASC"]],
        include: [
          {
            model: db.vehicle_brand,
            as: "brand",
            attributes: ["id", "name"],
          },
        ],
      })
      .then((vb) => {
        const data = {
          data: vb.rows || [],
          totalRows: vb.count || 0,
          page: offset ? offset : 1,
          totalPage: Math.ceil(vb.count / limit) || 1,
        };
        res.status(200).send(data);
      })
      .catch((e) => {
        console.error(e);
        res.status(400).send("Bad Request!");
      });
  });

  app.get("/api/vehicle-types/:id", [authJwt.verifyToken], (req, res) => {
    const id = parseInt(req.params.id);
    if (!id) res.status(400).send("Bad Request!");
    else {
      db.vehicle_type
        .findByPk(id, {
          include: [
            {
              model: db.vehicle_brand,
              as: "brand",
              attributes: ["id", "name"],
            },
          ],
        })
        .then((vb) => {
          if (vb) res.status(200).send(vb);
          else res.status(404).send({ message: "Data not found!" });
        })
        .catch((e) => {
          console.error(e);
          res.status(500).send("Server error!");
        });
    }
  });

  app.post(
    "/api/vehicle-types",
    [authJwt.verifyToken, authJwt.isAdmin],
    (req, res) => {
      const name = req.body.name;
      const brand_id = parseInt(req.body.brand_id);

      if (!name && !brand_id) res.status(400).send("Bad Request!");
      else {
        try {
          checkDuplicate(db.vehicle_type, {
            name: name,
            brand_id: brand_id,
          }).then((duplicated) => {
            if (!duplicated) {
              db.vehicle_type
                .create({ name: name, brand_id: brand_id })
                .then((vbNew) => {
                  res.status(200).send(vbNew);
                });
            } else res.status(422).send({ message: "Duplicated!" });
          });
        } catch (e) {
          res.status(500).send("Server error!");
          console.error(e);
        }
      }
    }
  );

  app.patch(
    "/api/vehicle-types/:id",
    [authJwt.verifyToken, authJwt.isAdmin],
    (req, res) => {
      const name = req.body.name;
      const brand_id = parseInt(req.body.brand_id);

      if (!name && !brand_id) res.status(400).send("Bad Request!");
      else {
        try {
          const id = parseInt(req.params.id);
          db.vehicle_type.findByPk(id).then((vb) => {
            if (vb) {
              let save = false;
              if (name && vb.name !== name) {
                vb.name = name;
                save = true;
              }
              if (brand_id && vb.brand_id !== brand_id) {
                vb.brand_id = brand_id;
                save = true;
              }

              if (save) {
                checkDuplicate(db.vehicle_type, {
                  name: vb.name,
                  brand_id: vb.brand_id,
                }).then((duplicated) => {
                  if (!duplicated) {
                    vb.save().then((vbNew) => {
                      res.status(200).send(vbNew);
                    });
                  } else res.status(422).send({ message: "Duplicated!" });
                });
              } else {
                res.status(200).send(vb);
              }
            } else res.status(404).send({ message: "Data not found!" });
          });
        } catch (e) {
          res.status(500).send("Server Error!");
          console.error(e);
        }
      }
    }
  );

  app.delete(
    "/api/vehicle-types/:id",
    [authJwt.verifyToken, authJwt.isAdmin],
    (req, res) => {
      const id = parseInt(req.params.id);
      if (!id) res.status(400).send("Bad Request!");
      else {
        try {
          db.vehicle_type.findByPk(id).then((vb) => {
            vb.destroy().then(() => {
              res.status(200).send({
                status: "Ok",
                message: `Deleted ${id} from vehicle_type`,
              });
            });
          });
        } catch (e) {
          res.status(500).send("Server Error!");
          console.error(e);
        }
      }
    }
  );
};
